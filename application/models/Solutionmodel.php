...
class Solutionmodel extends CI_Model{
    ...
    function _get_applicant_query(){
        $__order 			= array('solution_id' => 'DESC');
        $__column_search 	= array('solution_id', 'category_name', 'ticket_description', 'solution_desc', 'create_by', 'create_name');
        $__column_order     = array('solution_id', 'category_name', 'ticket_description', 'solution_desc', 'create_by', 'create_name');

        $this->db->select('*');
        $this->db->from('vw_solution');
