...
class User{
	...
	function getCdpt()		{return ($this->user) ? trim($this->user->log_cdpt) : NULL;} 	/*** Directorate ***/
	function getDirName()	{return ($this->user) ? trim($this->user->log_dir) : NULL;} 	/*** Directorate Name ***/
	function getCdic()		{return ($this->user) ? trim($this->user->log_cdic) : NULL;} 	/*** Division ***/
	function getDivName()	{return ($this->user) ? trim($this->user->log_div) : NULL;} 	/*** Division Name ***/
	function getCsdp()		{return ($this->user) ? trim($this->user->log_csdp) : NULL;} 	/*** Department ***/
	function getDept()		{return ($this->user) ? trim($this->user->log_dept) : NULL;} 	/*** Department Name ***/
	function getCdac()		{return ($this->user) ? trim($this->user->log_cdac) : NULL;} 	/*** Section ***/
	function getPost()		{return ($this->user) ? trim($this->user->log_post) : NULL;} 	/*** Jabatan / Position (add 22-02-2019 AP) ***/
